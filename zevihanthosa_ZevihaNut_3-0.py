...
def save(obj, wantdict=False):
    ...
    elif obj.type in [9, 10, 11, 12, 13]:
        data = obj.savedict()
    if not wantdict:
...
class Cell:
    ...
    def forget(self, force=0):
        self.mw = self.mw*force
        self.mb = self.mb*force
    ...
    def process(self, input, target=None, train=True, trainrate=1, learnrate=1, perceptron=False, naturalderiv=False):
        ...
        if not train:
            return output
        error = target-output
        learnrate = (self.learning*abs(error) if naturalderiv else self.learning)*learnrate
        if perceptron:
            self.mw = (self.momentumexc*self.mw)+((error*input)*trainrate)
            self.mb = (self.momentumexc*self.mb)+(error*trainrate)
            self.weight += self.mw*learnrate
            self.bias += self.mb*learnrate
        else:
...
class MultiInputCell:
    ...
    def limitation(self, limit=512):
        ...
        self.bias = max(min(self.bias, limit), -limit)
    def forget(self, force=0):
        self.mw = [m*force for m in self.mw]
        self.mb = self.mb*force
    def rmtolerance(self, sensitivity=128):
    ...
    def quantization(self, sensitivity=128):
        ...
        self.learning = max(quantization(self.learning, sensitivity=sensitivity), 1/sensitivity)
        self.truely = max(quantization(self.truely, sensitivity=sensitivity), 1/sensitivity)
    def process(self, inputs, target=None, train=True, trainrate=1, learnrate=1, perceptron=False, naturalderiv=False):
        ...
        if perceptron:
            for i in range(self.wcount):
                ...
                self.weights[i] += self.mw[i]*learnrate
            self.mb = (self.momentumexc*self.mb)+(error*trainrate)
            self.bias += self.mb*learnrate
        else:
            deriv = self.activation.derivation(output)
...
class MultiOutputCell:
    ...
    def forget(self, force=0):
        self.mw = [m*force for m in self.mw]
        self.mb = [m*force for m in self.mb]
    def rmtolerance(self, sensitivity=128):
        self.weights = [rmtolerance(w, sensitivity=sensitivity) for w in self.weights]
        ...
        for i in range(self.pcount):
            ...
            if train:
                ...
                if perceptron:
                    ...
                    self.biases[i] += self.mb[i]*learnrate
                else:
                    deriv = self.activation.derivation(output)
                    delta = error*deriv
                    graw = delta*input
...
class MultiCell:
    ...
    def forget(self, force=0):
        self.mw = [[m*force for m in ms] for ms in self.mw]
        self.mb = [m*force for m in self.mb]
    def rmtolerance(self, sensitivity=128):
        for i in range(self.ocount):
            ...
            if train:
                ...
                if perceptron:
                    for j in range(self.icount):
                        ...
                        self.weights[i][j] += self.mw[i][j]*learnrate
                    self.mb[i] = (self.momentumexc*self.mb[i])+(error*trainrate)
                    self.biases[i] += self.mb[i]*learnrate
                else:
                    deriv = self.activation.derivation(output)
...
class NanoCell:
    ...
    def process(self, input, target=None, train=True, trainrate=1, learnrate=1, perceptron=False, naturalderiv=False):
        ...
        learnrate = (self.learning*abs(error) if naturalderiv else self.learning)*learnrate
        if perceptron:
            self.weight += error*input*learnrate*trainrate
            self.bias += error*learnrate*trainrate
        else:
            deriv = self.activation.derivation(output)
            delta = error*deriv
            graw = delta*input
            grab = delta*self.truely
            self.weight += graw*learnrate*trainrate
            self.bias += grab*learnrate*trainrate
        return output
...
class CellForest:
    ...
    def process(self, input, target=None, train=True, trainrate=1, learnrate=1, perceptron=False, naturalderiv=False, top_k=4, pointweight=True):
        results = []
        main = 0.5
        cells = self.get_target_cells(input, top_k=top_k)
        best_cell = cells[0][0]
        for cell in cells:
            result = cell[0].process(input, target=target, train=train, trainrate=trainrate*cell[1], learnrate=learnrate, perceptron=perceptron, naturalderiv=naturalderiv)
            if cell[0] == best_cell:
                main = result
...
class MultiCellForest:
    ...
    def get_target_cells(self, inp, top_k=4, clusters=32):
        ...
        cells = cells[:top_k]
        return [[cell[0], 1-(i/len(cells))] for i, cell in enumerate(cells)]
    def process(self, input, target=None, train=True, trainrate=1, learnrate=1, perceptron=False, naturalderiv=False, top_k=4, clusters=32, pointweight=True):
        results = []
...
class CellNetwork:
    ...
    def process(self, input, target=None, train=True, trainrate=1, learnrate=1, perceptron=False, naturalderiv=False):
        ...
        for i in reversed(range(len(self.layers))):
            ...
            for j, cell in enumerate(current_layer):
                cell_error = errors[j]
                cell.process(prev_layer_acts, target=cell_error+layer_outs[i+1][j], train=True, trainrate=trainrate, learnrate=learnrate, perceptron=perceptron, naturalderiv=naturalderiv)
    ...
    def loaddict(self, data):
        ...
        for d, a in zip(data["a"], self.activations):
            a.load(d)
        self.update_activations(self.activations)
...
class CellNetworkForest:
    ...
    def process(self, input, target=None, train=True, trainrate=1, learnrate=1, perceptron=False, naturalderiv=False, top_k=4, clusters=32, pointweight=True):
        ...
        best_network = ns[0][0]
        for network in ns:
            result = network[0].process(input, target=target, train=train, trainrate=trainrate*network[1], learnrate=learnrate, perceptron=perceptron, naturalderiv=naturalderiv)
...
class Brain:
    ...
    def limitation(self, limit=512):
        for lobe in self.lobes:
            lobe.limitation(limit=limit)
    def forget(self, force):
        for lobe in self.lobes:
            lobe.forget(force)
    def rmtolerance(self, sensitivity=128):
        for lobe in self.lobes:
            lobe.rmtolerance(sensitivity=sensitivity)
    def quantization(self, sensitivity=128):
        for lobe in self.lobes:
            lobe.quantization(sensitivity=sensitivity)
    def update_activations(self, activations):
        for i, acts in enumerate(activations):
            self.lobes[i].update_activations(acts)
        self.activations = activations
    def process(self, input, target=None, train=True, trainrate=1, learnrate=1, perceptron=False, naturalderiv=False):
        if target is None:
            target = [0.5]*self.rawlobes[-1][-1]
        layers = sum([lobe.layers for lobe in self.lobes], start=[])
        layersset = sum([[self.lobesset[i]]*len(lobe.layers) for i, lobe in enumerate(self.lobes)], start=[])
        rawlayers = [self.lobes[0].rawlayers[0]]
        for lobe in self.lobes:
            rawlayers.extend(lobe.rawlayers[1:])
        current = input
        layer_outs = [current]
        for layer in layers:
            outs = []
        ...
        for i in reversed(range(len(layers))):
            next_layer_errors = [0.0]*rawlayers[i]
            current_layer = layers[i]
            prev_layer_acts = layer_outs[i]
            for j, cell in enumerate(current_layer):
                cell_error = errors[j]
                cell.process(prev_layer_acts, target=cell_error+layer_outs[i+1][j], train=True, trainrate=trainrate*(layersset[i][0]), learnrate=learnrate*(layersset[i][1]), perceptron=perceptron, naturalderiv=naturalderiv)
                for k in range(len(cell.weights)):
                    next_layer_errors[k] += cell.weights[k]*cell_error
            errors = next_layer_errors
        return final_out
    def savedict(self):
        return {"t": 13, "ls": [save(lobe, wantdict=True) for lobe in self.lobes], "rls": self.rawlobes, "lbs": self.lobesset, "a": [[a.save() for a in acts] for acts in self.activations]}
    def loaddict(self, data):
        self.lobes = [load(dat, wantdict=True) for dat in data["ls"]]
        self.rawlobes = data["rls"]
        self.lobesset = data["lbs"]
        self.activations = [[Activation() for _ in range(len(acts))] for acts in data["a"]]
