...
class CellForest:
    ...
    def process(self, input, target=None, train=True, trainrate=1, perceptron=False, distance_sharpness=8, top_k=4, pointweight=True):
        results = []
        main = 0.5
        for cell in self.get_target_cells(input, distance_sharpness=distance_sharpness, top_k=top_k):
...
class MultiCellForest:
    ...
    def savedict(self):
        return {"t": 10, "cs": [save(cell, wantdict=True) for cell in self.cells], "a": self.activation.save(), "ic": self.icount, "oc": self.ocount}
    def loaddict(self, data):
        self.cells = [load(cell, wantdict=True) for cell in data["cs"]]
        self.cellscount = len(data["cs"])
        self.activation.load(data["a"])
...
class CellNetworkForest:
    def __init__(self, networkcount=32, layers=[1, 16, 1], activations=None, learning=None, truely=1, momentumexc=0.9):
        if activations is None:
            activations = ["s"]*len(layers)
        self.networks = [CellNetwork(layers=layers, learning=learning, truely=truely, momentumexc=momentumexc) for _ in range(networkcount)]
    ...
    def get_target_networks(self, inp, distance_sharpness=8, top_k=4):
        ...
        for single_inp in inp:
            inpr = quantization_point(single_inp, sensitivity=self.networkcount)
            for i in range(self.networkcount):
                seq = 1 - (abs(i - inpr) / self.networkcount)
                rat = distance_weighted_ratio(seq, sharpness=distance_sharpness)
                if rat > 0:
                    networks_with_scores.append((i, rat, single_inp))
        aggregated = defaultdict(float)
        for idx, score, _ in networks_with_scores:
            aggregated[idx] += score
        sorted_networks = sorted(aggregated.items(), key=lambda x: x[1], reverse=True)
        result = []
        for idx, total_score in sorted_networks[:top_k]:
            if total_score > 0:
    ...
    def process(self, input, target=None, train=True, trainrate=1, perceptron=False, distance_sharpness=8, top_k=4, pointweight=True):
        ...
        for network in self.get_target_networks(input, distance_sharpness=distance_sharpness, top_k=top_k):
        ...
        return [(i+i2)/2 for i, i2 in zip(result, main)] if pointweight else result
    def savedict(self):
        return {"t": 12, "ns": [save(network, wantdict=True) for network in self.networks], "ls": self.layers, "a": self.activation.save()}
    def loaddict(self, data):
        self.networks = [load(network, wantdict=True) for network in data["ns"]]
        self.networkcount = len(data["ns"])
        self.layers = data["ls"]
        self.activation.load(data["a"])
        self.update_activation(self.activation)
