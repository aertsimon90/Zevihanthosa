...
def load(data, wantdict=False):
    ...
    if t == 0:
        ...
        obj = MultiCellForestNetwork(layers=[0])
        obj.loaddict(data)
    return obj
def copy(obj):
...
class MultiCellForestNetwork: # MCFN
    def __init__(self, layers=[1, 16, 1], percellscount=32, learning=None, truely=1, momentumexc=0.9):
        self.layers = []
        self.rawlayers = layers
        for i in range(len(layers)-1):
            input_count = layers[i]
            output_count = layers[i+1]
            self.layers.append([MultiCellForest(cellscount=percellscount, icount=input_count, ocount=1, learning=learning, truely=truely, momentumexc=momentumexc) for _ in range(output_count)])
        self.activation = Activation()
        self.update_activation(self.activation)
        self.type = 12
    def limitation(self, limit=512):
        for layer in self.layers:
            for cell_forest in layer:
                cell_forest.limitation(limit=limit)
    def update_activation(self, obj):
        self.activation = obj
        for layer in self.layers:
            for cell_forest in layer:
                cell_forest.update_activation(obj)
    def process(self, inputs, target=None, train=True, trainrate=1, perceptron=False, distance_sharpness=8, pointweight=True):
        if target is None:
            target = [0.5] * self.rawlayers[-1]
        current = inputs   
        layer_outs = [current]
        for layer in self.layers:
            outs = []
            for cell_forest in layer: 
                out = cell_forest.process(current, train=False, distance_sharpness=distance_sharpness, pointweight=pointweight)[0] 
                outs.append(out)
            current = outs
            layer_outs.append(current)
        final_out = current
        ...
        for i in reversed(range(len(self.layers))): 
            ...
            current_layer = self.layers[i]
            prev_layer_acts = layer_outs[i]             
            for j, cell_forest in enumerate(current_layer):
                cell_error = errors[j]
                adjusted_target = [cell_error + layer_outs[i+1][j]]
                cell_forest.process(prev_layer_acts, target=adjusted_target, train=True, trainrate=trainrate, perceptron=perceptron, distance_sharpness=distance_sharpness, pointweight=pointweight)
                total_weights_per_input = [0.0] * self.rawlayers[i]
                for single_cell in cell_forest.cells:
                    for k in range(len(single_cell.weights[0])):
                        total_weights_per_input[k] += single_cell.weights[0][k]
                avg_weights = [w / len(cell_forest.cells) for w in total_weights_per_input]
                for k in range(self.rawlayers[i]):
                    next_layer_errors[k] += avg_weights[k] * cell_error
            errors = next_layer_errors
        return final_out
    def savedict(self):
        return {"t": 12, "rl": self.rawlayers, "l": [[save(cell, wantdict=True) for cell in layer] for layer in self.layers], "a": self.activation.save()}
    def loaddict(self, data):
        self.layers = [[load(cell, wantdict=True) for cell in layer] for layer in data["l"]]
        self.rawlayers = data["rl"]
        self.activation.load(data["a"])
